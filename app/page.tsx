...
`return data[0].sma20 > data[0].sma50 && data[1].sma20 < data[1].sma50`,
  ...
  const App = () => {
    ...
    //   }[]
    // >([]);
    const [currentMarkerSnapshotIndex, setCurrentMarkerSnapshotIndex] =
      useState<number>(0);
    ...
    const [currentSeries, setCurrentSeries] =
      useState<UserSeries>(DEFAULT_USER_SERIES);
    ...
    const handleFetchClick = async () => {
      ...
      try {
        ...
        const rawData = await fetchCandlesFromMemory(
          ticker,
          timeframe,
          startDate.getTime(),
          endDate.getTime()
        ...
        const consolidatedSeries = getConsolidatedSeries(
          formattedCandles,
          newUserSeriesData
        );
        ...
        if (userOutcome) {
          const outcomeUserFunc = new Function(
            'data',
            'trigger',
            userOutcome.outcomeFunctionString,
          );
          ...
          const { outcomes, summary } = calculateOutcomes({
            ...
            triggers: conditionMarkers.map((condition) => {
              ...
              return {
                ...
                offset: consolidatedSeries.findIndex(
                  (c) => c.time === condition.marker.time
                ),
          ...
          setOutcomeMarkers(
            calculatedOutcomeMarkers.map((outcome) => outcome.marker)
          );
          setOutcomeSummary({
        ...
        setMarkerSnapshots(
          conditionMarkers.map((marker) => {
            ...
            const outcome = calculatedOutcomeMarkers.find(
              (outcome) => outcome.marker.text === marker.marker.text
            );
            if (outcome) {
              result.outcome = {
                outcomeDetails: {
      ...
      <>
                ...
                <div className={'flex flex-col w-[300px] gap-3'}>
                  ...
                  {userSeries.map((series) => (
                    ...
                    >
                      <div
                        className={
                          'flex flex-col flex-auto bg-primary-bg rounded-lg p-2'
                        }
                      >
                        <div className={'flex flex-row gap-5 justify-between'}>
                          ...
                          <div className={'flex flex-row items-center gap-2'}>
                            ...
                            <Checkbox
                              ...
                              onClick={() => {
                                const index = userSeries.findIndex(
                                  (s) => s.name === series.name
                                );
                                const newSeries = [...userSeries];
                                newSeries[index].overlay =
                                  !newSeries[index].overlay;
                        ...
                        <div className={'flex flex-row gap-5 justify-between'}>
                          ...
                          <div className={'flex flex-row items-center gap-3'}>
                            <IconButton
                              variant={'ghost'}
                              onClick={() => {
                                const index = userTriggers.findIndex(
                                  (s) => s.name === trigger.name
                                );
        ...
        <UserTriggerDialog
          ...
          onSaveClicked={() => {
            ...
            const triggerIndex = updatedTriggers.findIndex(
              (t) => t.id === currentTrigger.id
            );
